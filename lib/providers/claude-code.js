...
/**
 * Collect Anthropic content blocks from typed content, recursing through
 * nested tool results. Images read through the durable attachment service
 * (`ctx.get("attachments")`) become Anthropic base64 source blocks; nested
 * tool-result text is flattened into it (the Anthropic wire has no nested
 * tool_result blocks). Throws UNSUPPORTED_CONTENT when an image is present but
 * the attachment service is unavailable, instead of silently dropping pixels.
 */
async function serializeContentBlocks(blocks, attachments) {
  const out = [];
  for (const block of Array.isArray(blocks) ? blocks : []) {
    ...
    if (block.type === "text" && typeof block.text === "string" && block.text.length > 0) {
      ...
      if (!attachments) throw new LlmError("claude-code: image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
      ...
      out.push(...(await serializeContentBlocks(block.content, attachments)));
    }
  }
  return out;
}

/** One tool_result's content: plain string when text-only (existing wire shape), block array when it carries images. */
async function toolResultContent(blocks, attachments) {
  const parts = await serializeContentBlocks(blocks, attachments);
  if (parts.length === 0) return "(no output)";
  if (parts.every((part) => part.type === "text")) return parts.map((part) => part.text).join("");
  return parts;
}

async function serializeUserAnthropic(message, attachments) {
  const blocks = [];
  const text = flattenText(message.content);
  if (text.length > 0) blocks.push({ type: "text", text });
  for (const block of Array.isArray(message.content) ? message.content : []) {
    if (block?.type === "image") {
      ...
      blocks.push({
        ...
        content: await toolResultContent(block.content, attachments),
...
async function convertToAnthropicMessages(messages = [], attachments) {
  const wire = [];
  for (const message of messages) {
    if (!message || message.role === "system") continue;
    if (message.role === "assistant") wire.push(serializeAssistantAnthropic(message));
    else if (message.role === "user") wire.push(await serializeUserAnthropic(message, attachments));
...
function reasoningInfo(modelId) {
  ...
  return {
    // Match the Claude Code CLI, which runs extended thinking by default at a
    // 1024-token budget. The harness core materializes `defaultEffort` into
    // every request (`effective = requested ?? reasoning.defaultEffort`), so
    // without it no `thinking` block is ever sent and the reasoning stream
    // stays empty until the user manually picks an effort.
    defaultEffort: "low",
...
async function buildAnthropicPayload(options, entry, attachments) {
  ...
  const request = {
    ...
    stream: true,
    messages: await convertToAnthropicMessages(options.messages, attachments),
    system,
...
async function* translateAnthropicSse(events, emptyResponseCode = EMPTY_RESPONSE_CODE) {
  ...
  const openBlock = (index, kind, id, blockName) => {
    const block = {
      ...
      closed: false,
      // Reasoning blocks are emitted lazily: the Claude subscription route
      // redacts extended thinking for some plan/model combinations by opening a
      // `thinking` (or `redacted_thinking`) block that carries only an empty
      // thinking_delta and an encrypted signature. Emitting such a block anyway
      // surfaces an empty "Think" disclosure in the UI (which renders a Think
      // row for any reasoning block). Defer block-start until the first
      // non-empty reasoning text actually arrives, and drop the block entirely
      // if none does. Text/tool blocks stay eager so a tool call with an empty
      // argument list is never dropped.
      deferred: kind === "reasoning",
      started: kind !== "reasoning",
    };
  ...
  /** Emit the terminal usage + finish chunks for this stream. */
  function* emitFinish() {
    if (usage && Object.keys(usage).length > 0) yield { type: "usage", usage };
    if (order.length === 0) {
      // Nothing streamed at all: genuine empty response.
      yield { type: "finish", reason: emptyFinishReason(stopReason, emptyResponseCode) };
    } else {
      // Normal completion. Blocks that were deferred (redacted/empty reasoning)
      // and never started are simply absent; a turn whose only content was
      // redacted thinking still completes cleanly instead of erroring.
      const emitted = order.filter((block) => block.started);
      yield { type: "finish", reason: finishReason(stopReason, emitted.some((block) => block.kind === "tool-call")) };
    }
  }
  ...
  for await (const record of events) {
    ...
    switch (type) {
      ...
      case "content_block_start": {
        ...
        // Deferred (reasoning) blocks start only once real text arrives.
        if (block.started) yield { type: "block-start", index: block.index, blockType: kind };
        break;
      }
      case "content_block_delta": {
        ...
        if (delta.type === "text_delta" && typeof delta.text === "string") {
          ...
          // Redacted thinking streams empty deltas; skip them so no empty
          // reasoning block (and no empty Think disclosure) is ever produced.
          if (delta.thinking.length === 0) break;
          if (block.deferred && !block.started) {
            block.started = true;
      ...
      case "content_block_stop": {
        ...
        if (block && !block.closed) {
          block.closed = true;
          // A deferred reasoning block that never received visible text is
          // dropped: no block-start was emitted, so emit no block-end either.
          if (block.started) yield { type: "block-end", index: block.index, block: closeBlock(block) };
        }
        break;
      }
      case "message_delta":
...
/**
 * Every catalog entry is a Claude model (the catalog filter only accepts
 * `claude-*` upstream ids) and the whole Claude family accepts image input, so
 * vision is claimed unconditionally here. This is what the harness image
 * admission gate (`selectModel`) checks before letting an image session use a
 * model — under-claiming refuses every Claude model in image sessions.
 */
...
class ClaudeCodeAdapter extends LlmAdapter {
  ...
  async listModels(provider) {
    if (this.config.isEnabled?.() === false) return [];
    return (await this.effectiveModels()).map((model) => modelInfo(provider, model));
  }

  async resolveModel(provider, model) {
    if (this.config.isEnabled?.() === false) throw new LlmError("claude-code: disabled in Seraphim settings — enable it in Settings → Seraphim", "DISABLED");
    const options = this.config.options();
    const entry = (await this.effectiveModels()).find((candidate) => candidate.id === model);
    return {
      ...(entry ? modelInfo(provider, entry) : {
        provider,
        id: model,
        name: model,
        // Unknown ids: claim vision only for Claude ids (the whole family is vision-capable).
        inputModalities: String(model).startsWith("anthropic/claude-") ? ["text", "image"] : ["text"],
      }),
  ...
  /** Resolve the current account without advancing the cursor. */
  peekAccount() {
  ...
  /** Resolve the current account and advance the cursor to the next one. */
  resolveAccount() {
    const account = this.peekAccount();
    const accounts = this.config.store?.accounts?.() ?? [];
    this.accountCursor = (this.accountCursor + 1) % Math.max(1, accounts.length);
    return account;
  }
  ...
  async request(entry, options, signal, attachments) {
    const accounts = this.config.store?.accounts?.() ?? [];
    if (accounts.length === 0) {
      const token = await this.ensureAccessToken(signal);
      return this.requestWithToken(entry, options, signal, token, attachments);
    }

    // Pick the primary account once (no cursor advance) and try it.
    const primaryAccount = this.peekAccount();
    let lastResponse;
    let lastError;
    try {
      const token = await this.ensureAccessToken(signal, false, primaryAccount);
      const response = await this.requestWithToken(entry, options, signal, token, attachments);
      lastResponse = response;
      if (response.ok) return response;
      // Only fall back to other accounts on auth failures (401/403).
      // Non-auth errors (rate limit, server) surface directly — rotating
      // accounts won't help and would wastefully burn another account's quota.
      if (response.status !== 401 && response.status !== 403) {
        const text = await readResponseText(response);
        let parsedError;
        try { parsedError = JSON.parse(text || "{}"); } catch { parsedError = undefined; }
        const message = requestErrorMessage(response.status, parsedError, text, response);
        throw new LlmError(
          `claude-code: API error (HTTP ${response.status}): ${message}`,
          httpErrorCode(response.status, text),
          { status: response.status, ...(parseRetryAfterMs(response) !== undefined ? { providerRetryAfterMs: parseRetryAfterMs(response) } : {}) },
        );
      }
      lastError = new LlmError(`claude-code: account rejected (HTTP ${response.status})`, "AUTH", { status: response.status });
    } catch (error) {
      lastError = error;
      if (!(error instanceof LlmError && (error.code === "AUTH" || error.code === "TRANSPORT"))) throw error;
    }

    // Primary account failed with auth/transport error — try the remaining accounts as fallback.
    const attempted = new Set([primaryAccount.accessToken]);
    for (let round = 0; round < accounts.length - 1; round++) {
      // Advance cursor to the next account for fallback attempts.
      const account = this.resolveAccount();
      if (attempted.has(account.accessToken)) {
        // Cursor already advanced by resolveAccount; this is fine for fallback.
        continue;
      }
      ...
      try {
        ...
        // Not an auth failure: surface it directly.
        const text = await readResponseText(response);
        let parsedError;
        try { parsedError = JSON.parse(text || "{}"); } catch { parsedError = undefined; }
        const message = requestErrorMessage(response.status, parsedError, text, response);
  ...
  async requestWithToken(entry, options, signal, token, attachments) {
    ...
    const body = JSON.stringify(await buildAnthropicPayload(options.request, entry, attachments));
    let response;
    try {
  ...
  async *stream(request) {
    ...
    try {
      ...
      const response = await this.request(entry, { ...options, request }, signal, attachments);
