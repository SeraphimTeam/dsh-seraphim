...
async function* translateAnthropicSse(events, emptyResponseCode = EMPTY_RESPONSE_CODE) {
  ...
  const openBlock = (index, kind, id, blockName) => {
    const block = {
      ...
      closed: false,
      // Reasoning blocks are emitted lazily: Claude Opus (and interleaved
      // thinking) can open a thinking block that carries only an encrypted
      // signature (redacted thinking) or an empty thinking_delta, with no
      // visible text. Emitting it anyway surfaces an empty "Think" disclosure
      // in the UI (which renders a Think row for any reasoning block). Defer
      // block-start until the first non-empty reasoning text actually arrives,
      // and drop the block entirely if none does. Text/tool blocks stay eager
      // so an empty-argument tool call is never dropped.
  ...
  const isEmittedBlock = (block) => !(block.deferred && !block.started);
    ...
    switch (type) {
      ...
      case "content_block_delta": {
        ...
        if (delta.type === "text_delta" && typeof delta.text === "string") {
          ...
          // Skip empty thinking deltas so a redacted/empty thinking block never
          // opens a "Think" disclosure with no content.
      ...
      case "content_block_stop": {
        ...
        if (block && !block.closed) {
          ...
          if (isEmittedBlock(block)) yield { type: "block-end", index: block.index, block: closeBlock(block) };
        }
        break;
      }
      case "message_delta":
      ...
      case "message_stop":
        ...
        if (!order.some(isEmittedBlock)) {
          yield {
            type: "finish",
            reason: emptyFinishReason(stopReason, emptyResponseCode),
          };
        } else {
          yield { type: "finish", reason: finishReason(stopReason, order.some((block) => isEmittedBlock(block) && block.kind === "tool-call")) };
        }
        return;
      default:
        break;
