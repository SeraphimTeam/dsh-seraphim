...
export async function fetchModelCatalog({
  ...
  // Dedupe by normalized id: the live endpoint may report the same model under
  // both `claude-*` and `anthropic/claude-*` forms (or overlap pagination
  // pages), and both normalize to the same `anthropic/<upstream>` id — the
  // picker renders every returned id, so a duplicate row would show twice.
  const normalized = [];
  const seenIds = new Set();
  for (const entry of records.map((record) => modelEntryFromCatalog(record, models, excludedModelIds)).filter(Boolean)) {
