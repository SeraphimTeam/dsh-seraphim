 ...
 * become Anthropic base64 source blocks; a missing service is an explicit
 * error, never a silent pixel drop.
 */
async function serializeContentBlocks(blocks, attachments) {
  ...
  for (const block of Array.isArray(blocks) ? blocks : []) {
    ...
    if (block.type === "text" && typeof block.text === "string" && block.text.length > 0) {
      ...
      if (!attachments) throw new LlmError("zed: image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
...
/** One tool_result's content: plain string when text-only, block array when it carries images. */
...
async function buildAnthropicProviderRequest(options, entry, attachments) {
  const request = {
  ...
  request.messages = await convertToAnthropicMessages(options.messages, attachments);
  return request;
}
...
function buildProviderRequest(options, entry, attachments) {
  ...
  switch (provider) {
    ...
    default:
      return buildAnthropicProviderRequest(options, entry, attachments);
  }
}

/** Full /completions body for cloud.zed.dev. */
async function buildZedPayload(options, entry, attachments) {
  return {
    ...
    model: entry.upstream,
    provider_request: await buildProviderRequest(options, entry, attachments),
  };
}
...
/**
 * Only Anthropic-wire models can carry images here: cloud.zed.dev routes
 * `anthropic/*` (and unknown slugs) through an Anthropic Messages shape, while
 * openai/google/xai builders are text-only by design ("text only, per
 * zed2api"). The harness image admission gate checks inputModalities, so
 * under-claiming refuses Anthropic models in image sessions and over-claiming
 * would make text-only routes silently lose pixels.
 */
function modelModalities(modelId) {
  return providerForModel(modelId) === "anthropic" ? ["text", "image"] : ["text"];
}
...
function modelInfo(provider, model) {
  return {
    ...
    inputModalities: modelModalities(model.id),
...
class ZedAdapter extends LlmAdapter {
  ...
  resolveModel(provider, model, _signal) {
    if (this.config.isEnabled?.() === false) throw new LlmError("zed: disabled in Seraphim settings — enable it in Settings → Seraphim", "DISABLED");
    return this.effectiveModels().then((models) => {
      const configured = models.find((entry) => entry.id === model);
      return {
        ...(configured === undefined
          ? { provider, id: model, name: model, inputModalities: modelModalities(model), ...(reasoningInfo(model) === undefined ? {} : { reasoning: reasoningInfo(model) }) }
    ...
    try {
      for (const account of accounts) {
        ...
        try {
          ...
          const payload = await buildZedPayload(options, entry, attachments);
