...
class FreeBuffCredentialStore {
  ...
  listMeta() {
    return this.accounts().map((account, index) => ({
      ...
      admittedModel: typeof account.admittedModel === "string" ? account.admittedModel : undefined,
...
/**
 * Serialize harness messages into OpenAI chat-completions messages. User
 * content becomes a parts array when it carries images (standard `image_url`
 * data-URL parts); text-only messages keep the existing plain-string shape.
 * A missing attachment service is an explicit error, never a silent pixel drop.
 */
  ...
  for (const message of messages) {
    ...
    if (imageBlocks.length > 0) {
      ...
      for (const block of imageBlocks) {
        if (!attachments) throw new LlmError("freebuff: image input requires the durable attachment service", "UNSUPPORTED_CONTENT");
 ...
 */
async function buildUpstreamPayload(options, entry, session, runId, attachments) {
  const payload = {
    model: entry.upstream,
    messages: normalizeMessages([
      ...(options.system !== undefined ? [{ role: "system", content: options.system }] : []),
      ...(await serializeMessages(options.messages, attachments)),
    ]),
    stream: true,
...
async function probeAndStoreTier(store, baseURL, authToken) {
  ...
  if (typeof data?.model === "string" && data.model.length > 0) meta.admittedModel = data.model;
...
/**
 * Catalog entries whose upstream model accepts image input (cross-checked
 * against the Command Code vision registry; freebuff uses its own id format).
 * Only these claim `["text","image"]` — the admission gate and the wire must
 * agree, otherwise image sessions either get refused or silently lose pixels.
 */
...
class FreeBuffAdapter extends LlmAdapter {
  ...
  async ensureSession(authToken, sessionModel, signal) {
    ...
    // The upstream keeps ONE active session per account and binds it to the model
    // it admits. On the free tier that is the default free model and
    // `x-freebuff-model` is ignored at creation, so deleting the live session to
    // force a different model BURNS a daily session slot for nothing (the server
    // re-admits the same default model and chats still 409 session_model_mismatch).
    // Reuse the live instance and record the model the server actually bound; the
    // adapter falls back to it at chat time when the requested model differs.
    if (cur.status === 200 && cur.data?.status === "active" && cur.data?.instanceId) {
      const admitted = typeof cur.data.model === "string" && cur.data.model.length > 0 ? cur.data.model : sessionModel;
      const s = { instanceId: cur.data.instanceId, model: sessionModel, admittedModel: admitted, expiresAt: Date.now() + SESSION_TTL_MS };
  ...
  async createSession(authToken, sessionModel, signal) {
    ...
    if (r.status === 200 && r.data?.status === "active" && r.data?.instanceId) {
      const admitted = typeof r.data.model === "string" && r.data.model.length > 0 ? r.data.model : sessionModel;
      const s = { instanceId: r.data.instanceId, model: sessionModel, admittedModel: admitted, expiresAt: Date.now() + SESSION_TTL_MS };
      this.sessionCache.set(key, s);
      return s;
    }
    if (r.status === 200 && r.data?.status === "queued" && r.data?.instanceId) {
      ...
      for (let i = 0; i < SESSION_POLL_ATTEMPTS; i++) {
        ...
        if (q.status === 200 && q.data?.status === "active") {
          const admitted = typeof q.data.model === "string" && q.data.model.length > 0 ? q.data.model : sessionModel;
          const s = { instanceId: q.data.instanceId || inst, model: sessionModel, admittedModel: admitted, expiresAt: Date.now() + SESSION_TTL_MS };
          this.sessionCache.set(key, s);
          return s;
        }
      }
  ...
  async chatRequest(authToken, entry, session, payload, connection, options, signal) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      ...
      if (isLimitedAccess(errText)) {
        this.limited = true;
        if (LIMITED_MODELS.has(entry.session)) {
          // The requested model IS free-allowed, so "Limited free access" here
          // actually means the live session is bound to the OTHER free model.
          // Be specific instead of telling a flash user to "select one of those".
          const bound = session.admittedModel && session.admittedModel !== entry.session ? session.admittedModel : "another model";
          throw new LlmError(
            `freebuff: your account's session is bound to \`${bound}\`; the server is not admitting \`${entry.session}\` on this account right now — select ${bound === "another model" ? "the bound model" : "the bound model in the picker"} (or switch account)`,
            "INVALID_REQUEST",
            { status: response.status, accountRetryable: true, noCooldown: true },
          );
        }
        throw new LlmError(
    ...
    try {
      for (const authToken of tokens) {
        ...
        try {
          ...
          // Serve the model the account's session is actually admitted for: the
          // free tier binds sessions to its default model and 409s any chat for
          // a different one (session_model_mismatch). Falling back to the
          // admitted model keeps the turn working (e.g. a flash pick served by
          // the account's admitted mimo session) instead of erroring.
          const admittedEntry =
            typeof session.admittedModel === "string" &&
            session.admittedModel !== entry.session &&
            (connection.models.find((m) => m.session === session.admittedModel) ?? connection.models.find((m) => m.id === session.admittedModel));
          const chatEntry = admittedEntry ?? entry;
          const runId = await this.ensureRunChain(authToken, chatEntry.agent, signal);
          const attachments = this.config.resolveAttachments?.();
          const payload = await buildUpstreamPayload(options, chatEntry, session, runId, attachments);
          response = await this.chatRequest(authToken, chatEntry, session, payload, connection, options, signal);
        } catch (error) {
