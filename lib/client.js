window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const styles = {
      ...
      modelPane: { minWidth: 0, minHeight: 0, height: "100%", overflowY: "auto", overscrollBehaviorY: "contain", touchAction: "pan-y" },
      modelPaneHeader: {
        display: "flex",
        alignItems: "baseline",
        justifyContent: "space-between",
      ...
      modelList: { minHeight: 0, padding: 6, display: "flex", flexDirection: "column", gap: 2 },
      ...
      modelOptionWithEffort: { boxSizing: "border-box", minHeight: 44, width: "100%", display: "flex", alignItems: "center", gap: 6, padding: "4px 6px 4px 8px", borderRadius: 7 },
      modelOptionMain: { minWidth: 0, flex: 1, display: "flex", alignItems: "center", padding: "3px 0", border: 0, background: "transparent", color: "inherit", font: "inherit", textAlign: "left", cursor: "pointer" },
      modelEffortSelect: { flex: "0 0 auto", width: 112, height: 30, padding: "0 7px", border: "1px solid var(--dsw-alias-border-l2)", borderRadius: 6, background: "var(--dsw-specific-menu)", color: "var(--dsw-alias-label-primary)", font: "inherit", fontSize: 11 },
    };
    ...
    function BetterModelSelect({ locked, available, directory, load, select, t, copy = t }) {
      ...
      const choose = (selection) => {
        ...
        select(selection).then((accepted) => {
          if (!accepted) setSelectionError(directory.getSnapshot().error ?? copy("selectionFailed"));
        });
      };
      const retry = () => {
        setSelectionError("");
                              ...
                              children: [
                                ...
                                ...section.groups.map((group) => {
                                  ...
                                  return jsxs("button", {
                                    ref: itemRef,
                                    type: "button",
                                    className: `dsh-seraphim-model-row dsh-seraphim-provider-row${active ? " is-active" : ""}`,
                    ...
                    children: [
                      ...
                      activeProvider === undefined
                        ...
                        : jsxs("div", {
                              ...
                              className: "dsh-seraphim-model-list-appear",
                              style: styles.modelList,
                               ...
                               children: [
                                ...
                                activeProvider.models.map((model) => {
                                   ...
                                   const selectModel = () => choose({ provider: activeProvider.id, model: model.id, ...(reasoning?.defaultEffort === undefined ? {} : { reasoningEffort: reasoning.defaultEffort }) });
                                   const copyNode = jsxs("span", { style: styles.modelItemCopy, children: [jsx("span", { style: styles.modelName, children: model.name }), model.description ? jsx("span", { style: styles.modelDescription, children: model.description }) : null] });
                                   if (!selected || reasoning === undefined) return jsxs("button", {
                                     ref: itemRef,
                                     type: "button",
                                     className: `dsh-seraphim-model-row dsh-seraphim-model-option${selected ? " is-selected" : ""}`,
                                     style: { ...styles.modelOption, ...(selected ? styles.modelOptionSelected : {}) },
                                     disabled: busy,
                                     onClick: selectModel,
                                     children: [copyNode, jsx("span", { className: "dsh-seraphim-model-check", style: styles.modelCheck, children: selected ? jsx(ModelCheck, {}) : null })],
                                   }, `${activeProvider.id}:${model.id}`);
                                   const levels = [...(reasoning.defaultEffort === undefined ? [undefined] : []), ...reasoning.efforts.map((level) => level.id)];
                                   return jsxs("div", {
                                     className: `dsh-seraphim-model-row dsh-seraphim-model-option${selected ? " is-selected" : ""}`,
                                     style: { ...styles.modelOptionWithEffort, ...styles.modelOptionSelected },
                                     children: [
                                       jsx("button", { ref: itemRef, type: "button", style: styles.modelOptionMain, disabled: busy, onClick: selectModel, children: copyNode }),
                                       jsx("select", { "aria-label": copy("effort"), title: copy("effort"), style: styles.modelEffortSelect, disabled: busy, value: effectiveEffort ?? "", onChange: (event) => chooseEffort(event.target.value === "" ? undefined : event.target.value), children: levels.map((level) => jsx("option", { value: level ?? "", children: level === undefined ? copy("defaultEffort") : reasoning.efforts.find((candidate) => candidate.id === level)?.name ?? level }, level ?? "default")) }),
                                     ],
                                   }, `${activeProvider.id}:${model.id}`);
                                 }),
                              ],
                          ...
                          children: [
                            jsxs("div", {
                              ...
                              children: [
                                ...
                                account.admittedModel
                                  ? jsxs("div", {
                                      style: { display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" },
                                      children: [
                                        jsx("span", { style: styles.hint, children: t("admitted") }),
                                        jsx("span", { style: styles.planBadge, children: account.admittedModel }),
                                      ],
                                    })
                                  : null,
    ...
    function injectStyles() {
      ...
      tag.textContent = [
        ...
        ".dsh-seraphim-provider-row { transition: background-color 180ms cubic-bezier(.2,.8,.2,1), box-shadow 180ms cubic-bezier(.2,.8,.2,1), transform 180ms cubic-bezier(.2,.8,.2,1); }",
        ".dsh-seraphim-provider-row.is-active { transform: translateX(2px); }",
        ".dsh-seraphim-model-option { transition: background-color 160ms cubic-bezier(.2,.8,.2,1), box-shadow 160ms cubic-bezier(.2,.8,.2,1); }",
        ".dsh-seraphim-model-check { transition: opacity 160ms cubic-bezier(.2,.8,.2,1), transform 160ms cubic-bezier(.2,.8,.2,1); }",
        ".dsh-seraphim-model-option:not(.is-selected) .dsh-seraphim-model-check { opacity: 0; transform: scale(.82); }",
        ".dsh-seraphim-model-option.is-selected .dsh-seraphim-model-check { opacity: 1; transform: scale(1); }",
        ".dsh-seraphim-model-list-appear { animation: dsh-seraphim-model-list-in 180ms cubic-bezier(.2,.8,.2,1) both; }",
        "@keyframes dsh-seraphim-model-list-in { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }",
        ".dsh-seraphim-model-row:hover:not(:disabled) { background: var(--dsw-alias-interactive-bg-hover) !important; }",
        "@media (prefers-reduced-motion: reduce) { .dsh-seraphim-provider-row, .dsh-seraphim-model-option, .dsh-seraphim-model-check { transition: none !important; } .dsh-seraphim-provider-row.is-active { transform: none; } .dsh-seraphim-model-list-appear { animation: none !important; } }",
